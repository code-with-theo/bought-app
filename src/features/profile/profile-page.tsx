...
export function ProfilePage() {
  ...
  return <><AppShell><PageHeader eyebrow="我的" title="这份购物记忆" description="所有记录只保存在这台设备里。" /><section className="mt-7 grid grid-cols-3 gap-3">{loading ? <><LoadingState className="h-24"/><LoadingState className="h-24"/><LoadingState className="h-24"/></> : <><StatCard label="买过" value={total} /><StatCard label="会复购" value={repurchase} tone="success" /><StatCard label="避雷" value={avoid} tone="danger" /></>}</section>
    <ThemeSwitcher className="mt-8" /><section className="mt-8 space-y-3"><Button className="w-full" variant="secondary" onClick={() => void exportData()} disabled={!total}>{total ? "导出我的记录" : "还没有可导出的记录"}</Button>{demos ? <Button className="w-full" variant="secondary" onClick={() => setAction("demo")}>清除演示数据</Button> : null}</section><section className="mt-10 border-y border-danger py-5"><h2 className="font-semibold text-danger">谨慎操作</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">清空会删除所有记录、照片和评价，且无法恢复。</p><Button className="mt-4 w-full" variant="danger" onClick={() => setAction("all")}>清空全部数据</Button></section><section className="mt-8 border-y border-border py-5"><h2 className="font-semibold">关于「买过」</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">这是你的私人购物记忆库。记下感受，让下一次购买更值得。</p></section>
