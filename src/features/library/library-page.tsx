...
export function LibraryPage({ repurchaseOnly = false }: { repurchaseOnly?: boolean }) {
  ...
  return <><AppShell><PageHeader eyebrow={repurchaseOnly ? "买过" : "物品库"} title={title} description={description} />
    {!repurchaseOnly ? <><Input className="mt-6" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜商品名或品牌" aria-label="搜索商品" /><div className="mt-3 flex gap-2 overflow-x-auto pb-1">{([ ["all", "全部"], ["repurchase", "会复购"], ["avoid", "不会复购"], ["waiting", "等待评价"] ] as Array<[Quick, string]>).map(([value, label]) => <button type="button" key={value} onClick={() => setQuick(value)} className={`min-h-10 shrink-0 rounded-full px-4 text-sm font-bold ${quick === value ? "bg-primary text-white" : "bg-surface-muted text-muted-foreground"}`}>{label}</button>)}</div>
