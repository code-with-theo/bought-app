  ...
  return <><AppShell><PageHeader eyebrow={productId ? "编辑记录" : "新的记忆"} title={productId ? "改改这条记录" : "记一件买过的"} description="先留下最重要的三项，其余以后再补。" />
    <form className="mt-7 space-y-6" onSubmit={submit} noValidate>
      ...
      <div><FieldLabel>拍张照，方便下次认出来</FieldLabel><input ref={fileRef} type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" onChange={(event) => pickImage(event.target.files?.[0])} />
        {fields.image ? <div className="relative border-y border-border py-3"><ProductImage image={fields.image} name={fields.name || "商品"} category={fields.category} className="mx-auto aspect-[4/3] max-h-80 w-full max-w-lg" /><div className="absolute inset-x-3 bottom-6 flex gap-2"><Button type="button" variant="secondary" size="small" onClick={() => fileRef.current?.click()} disabled={imageBusy}>换一张</Button><Button type="button" variant="secondary" size="small" onClick={() => change("image", null)} disabled={imageBusy}>删除</Button></div></div> : <button type="button" onClick={() => fileRef.current?.click()} className="flex min-h-40 w-full flex-col items-center justify-center border-y border-dashed border-border py-8 text-muted-foreground hover:bg-surface-muted"><span className="text-2xl">＋</span><span className="mt-2 text-sm font-semibold">{imageBusy ? "图片处理中…" : "选择商品照片"}</span></button>}</div>
