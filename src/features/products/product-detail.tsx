...
export function ProductDetail() {
  ...
  return <><AppShell><PageHeader eyebrow={categoryLabels[product.category]} title={product.name} action={<Link className="text-sm font-bold text-primary" href={`/products/${product.id}/edit`}>编辑</Link>} />
    ...
    <section className="mt-7 border-y border-border py-5"><SectionHeader title="购买信息" />{info.filter(([, value]) => value).map(([label, value]) => <div className="mt-3 flex justify-between gap-4 text-sm" key={label}><span className="text-muted-foreground">{label}</span><span>{value}</span></div>)}</section>
