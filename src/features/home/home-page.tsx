...
export function HomePage() {
  ...
  if (!products.length) return <><AppShell><PageHeader eyebrow="买过" title="你的购物记忆" description="记下每一次选择，让下一次购买更值得。" /><div className="mt-8"><EmptyState title="还没有买过的记录" description="下次买到喜欢的，记在这里吧。" /></div></AppShell><BottomNavigation active="home" /></>;
  ...
  return <><AppShell><PageHeader eyebrow="买过" title="你的购物记忆" description="记下每一次选择，让下一次购买更值得。" />
    ...
    {repurchase.length ? <section className="mt-8"><SectionHeader title="值得复购" action={<button type="button" className="text-sm font-bold text-primary" onClick={() => router.push("/repurchase")}>查看全部</button>} /><div className="mt-3 flex gap-3 overflow-x-auto pb-2">{repurchase.slice(0, 5).map((product) => <ProductCard key={product.id} product={product} onClick={() => router.push(`/product?id=${encodeURIComponent(product.id)}`)} className="w-40 shrink-0" />)}</div></section> : null}
    ...
    {waiting.length ? <section className="mt-8"><SectionHeader title="等你来评价" /><div className="mt-3 space-y-3">{waiting.slice(0, 3).map((product) => <ProductListItem key={product.id} product={product} onClick={() => router.push(`/product?id=${encodeURIComponent(product.id)}`)} />)}</div></section> : null}
