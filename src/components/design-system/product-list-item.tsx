...
export function ProductListItem({ product, className, onClick }: { product: ProductRecord; className?: string; onClick?: () => void }) {
  ...
  const classes = cn("flex w-full items-center gap-3 rounded-[var(--radius-md)] bg-surface p-2 text-left shadow-[var(--shadow-soft)] hover:bg-surface-muted", className);
